...
export default new Vuex.Store({
  ...
  actions: {
    async getRouter (context) {
      const { data } = await getRoleByID(context.state.user.user_id)
      console.log(data)
    ...
    generationRouter ({ commit }, data) {
      console.log('generationRouter中', data)
      const roles = data
      console.log('allRoleRoutes值为', allRoleRoutes)
      const accessdRouters = allRoleRoutes.filter((item) => {
        roles.map((r) => {
          console.log('role值为', r)
          if (item.meta.role.indexOf(r) >= 0) {
            console.log(item.meta.role)
            console.log(item.meta.role.indexOf(r))
            return item
          }
        })
      })
      console.log('accessdRouters为', accessdRouters)
